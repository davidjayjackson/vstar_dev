...
mydata$yrmonth <- paste0(lubridate::year(mydata$Ymd), " ", lubridate::month(mydata$Ymd))
...
selectInput(inputId ="DropDownStar",label="Please Enter Star Name",
              choices = unique(mydata$star_name),selected="R AQL")

selectInput(inputId ="DropDownBand",label="Please Select Band",
              choices =unique(mydata$band),selected="Vis.")

selectInput(inputId ="DropDownYear",label="Select starting  Year",
              choices =unique(mydata$Year),selected=2000)
...
renderPlotly({
  alf_ori() %>%
    mutate(yrmonth = fct_reorder(yrmonth,Month),
           yrmonth = fct_reorder(yrmonth,Year)) %>%
    plot_ly() %>%
    add_boxplot(x=~yrmonth,y=~magnitude)
})
...
# alf_ori_monthly <-reactive({ alf_ori() %>%
#   group_by(year_month=yrmonth) %>%
...
# More magic by Tan @ Slack

alf_ori_monthly <-reactive({ 
  alf_ori() %>%
    group_by(year_month=yrmonth,Year,Month) %>%
    summarize(Mean = round(mean(magnitude),digits=1),
               Brighter = round(Mean -1,digits = 1),
               Fainter = round(Mean +1,digits= 1),
               Obs = n()) %>%
    ...
    mutate(Verify = case_when(
      (Brighter <  Mean -1) | (Fainter > Mean + 1) ~ "Yes",TRUE ~ "No")) %>%
    filter(Verify =="Yes") %>% arrange(Year,Month)
})
